...
void PmergeMe::sort()
{
	// Time vector sort
	{
		...
		_vecTime = static_cast<double>(end - start) / CLOCKS_PER_SEC * 1e6; // microseconds
	}
	// Time deque sort
	{
		std::clock_t start = std::clock();
		fordJohnsonDeq(_deq);
		std::clock_t end = std::clock();
		_deqTime = static_cast<double>(end - start) / CLOCKS_PER_SEC * 1e6;
	}
}
...
double PmergeMe::timeVec() const  { return _vecTime; }
double PmergeMe::timeDeq() const  { return _deqTime; }
